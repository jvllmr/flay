/*
    This is a one-to-one copy of rustpython_ast's unparse module.
    It is copied here because the original module as no public member.
    https://github.com/RustPython/Parser/blob/5e9d9853e751240b28138efd165e793a7ee5dad4/ast/src/unparse.rs
*/
use std::fmt;
...
impl<'a> Unparser<'a> {
    ...
    fn unparse_expr<U>(&mut self, ast: &Expr<U>, level: u8) -> fmt::Result {
        ...
        match &ast {
            Expr::BoolOp(rustpython_ast::ExprBoolOp {
                op,
                values,
                range: _range,
            }) => {
            ...
            Expr::NamedExpr(rustpython_ast::ExprNamedExpr {
                target,
                value,
                range: _range,
            }) => {
            ...
            Expr::BinOp(rustpython_ast::ExprBinOp {
                left,
                op,
                right,
                range: _range,
            ...
            Expr::UnaryOp(rustpython_ast::ExprUnaryOp {
                op,
                operand,
                range: _range,
            }) => {
            ...
            Expr::Lambda(rustpython_ast::ExprLambda {
                args,
                body,
                range: _range,
            }) => {
            ...
            Expr::IfExp(rustpython_ast::ExprIfExp {
                test,
                body,
                orelse,
                range: _range,
            ...
            Expr::Dict(rustpython_ast::ExprDict {
                keys,
                values,
                range: _range,
            }) => {
            ...
            Expr::Set(rustpython_ast::ExprSet {
                elts,
                range: _range,
            }) => {
                self.p("{")?;
            ...
            Expr::ListComp(rustpython_ast::ExprListComp {
                elt,
                generators,
                range: _range,
            }) => {
            ...
            Expr::SetComp(rustpython_ast::ExprSetComp {
                elt,
                generators,
                range: _range,
            }) => {
            ...
            Expr::DictComp(rustpython_ast::ExprDictComp {
                key,
                value,
                generators,
                range: _range,
            ...
            Expr::GeneratorExp(rustpython_ast::ExprGeneratorExp {
                elt,
                generators,
                range: _range,
            }) => {
            ...
            Expr::Await(rustpython_ast::ExprAwait {
                value,
                range: _range,
            }) => {
            ...
            Expr::Yield(rustpython_ast::ExprYield {
                value,
                range: _range,
            }) => {
            ...
            Expr::YieldFrom(rustpython_ast::ExprYieldFrom {
                value,
                range: _range,
            }) => {
                write!(self, "(yield from {})", **value)?;
            }
            Expr::Compare(rustpython_ast::ExprCompare {
                left,
                ops,
                comparators,
                range: _range,
            ...
            Expr::Call(rustpython_ast::ExprCall {
                func,
                args,
                keywords,
                range: _range,
            ...
            Expr::FormattedValue(rustpython_ast::ExprFormattedValue {
                value,
                conversion,
                format_spec,
                range: _range,
            }) => self.unparse_formatted(value, *conversion, format_spec.as_deref())?,
            Expr::JoinedStr(rustpython_ast::ExprJoinedStr {
                values,
                range: _range,
            ...
            Expr::Attribute(rustpython_ast::ExprAttribute { value, attr, .. }) => {
                self.unparse_expr(value, precedence::ATOM)?;
                let period = if let Expr::Constant(rustpython_ast::ExprConstant {
            ...
            Expr::Subscript(rustpython_ast::ExprSubscript { value, slice, .. }) => {
                self.unparse_expr(value, precedence::ATOM)?;
            ...
            Expr::Starred(rustpython_ast::ExprStarred { value, .. }) => {
                self.p("*")?;
                self.unparse_expr(value, precedence::EXPR)?;
            }
            Expr::Name(rustpython_ast::ExprName { id, .. }) => self.p_id(id)?,
            Expr::List(rustpython_ast::ExprList { elts, .. }) => {
                self.p("[")?;
                let mut first = true;
                for elt in elts {
                    self.p_delim(&mut first, ", ")?;
            ...
            Expr::Tuple(rustpython_ast::ExprTuple { elts, .. }) => {
                if elts.is_empty() {
                    self.p("()")?;
                } else {
                    group_if!(precedence::TUPLE, {
            ...
            Expr::Slice(rustpython_ast::ExprSlice {
                lower,
                upper,
                step,
                range: _range,
    ...
    fn unparse_fstring_elem<U>(&mut self, expr: &Expr<U>, is_spec: bool) -> fmt::Result {
        match &expr {
            Expr::Constant(rustpython_ast::ExprConstant { value, .. }) => {
